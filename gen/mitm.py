...
class Mitm:
    ...
    def _good_paths(self, x, y, dx, dy, budget, seen=None):
        ...
        if (x1, y1) not in seen:
            for path, end in self._good_paths(x1, y1, -dy, dx, budget-self.lr_price, seen):
                yield (L,) + path, end
            for path, end in self._good_paths(x1, y1, dy, -dx, budget-self.lr_price, seen):
                yield (R,) + path, end
            ...
            if (x2, y2) not in seen:
                for path, end in self._good_paths(x2, y2, dx, dy, budget-self.t_price, seen):
                    yield (T,) + path, end
            seen.remove((x1, y1)) # Clean up (B)
        seen.remove((x, y)) # Clean up (A)

    def rand_path(self, xn, yn, dxn, dyn):
        """ Returns a path, starting at (0,0) with dx0,dy0
        ...
        while True:
            ...
            # it to (dx, dy) it ends at xn-x, yn-y in direction dxn, dyn.
            xt, yt = unrotate(xn-x, yn-y, dx, dy)
            dxt, dyt = unrotate(dxn, dyn, dx, dy)
            path2s = self.inv[xt, yt, dxt, dyt]
            if path2s:
    ...
    def rand_loop(self, clock=0):
        ...
        while True:
            ...
            xt, yt = unrotate(-x, -y, dx, dy)
            dxt, dyt = unrotate(0, 1, dx, dy)
            path2s = self.inv[xt, yt, dxt, dyt]
            if path2s:
                path2 = random.choice(path2s)
                joined = Path(path+path2)
